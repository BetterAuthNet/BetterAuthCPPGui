...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	RenderTextClipped(ImVec2(frame_bb.Max.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x+window->Size.x- frame_bb.GetWidth()-CalcTextSize(value_buf_end).x, total_bb.Max.y), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (hovered)
	{
		...
		ImVec4 col = GetStyleColorVec4(ImGuiCol_CheckMark);
		col.w = 0.5f;
		RenderCheckMark(check_bb.Min + ImVec2(pad, pad), GetColorU32(col), check_bb.GetWidth() - pad * 2.0f);
	}
	...
	if (*v)
	{
		...
		border_col = ImColor(0, 221, 255, 255);
	}
	else
		border_col = GetStyleColorVec4(ImGuiCol_Border);

	window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(border_col), style.FrameRounding, ImDrawCornerFlags_All, 1.0f);

	if (g.LogEnabled)
		LogRenderedText(&text_bb.Min, *v ? xorstr("[x]") : xorstr("[ ]"));
	if (label_size.x > 0.0f)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	const ImRect value_bb(frame_bb.Min, frame_bb.Max);
	RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
	window->DrawList->AddRect(frame_bb.Min, frame_bb.Max+ImVec2(0,1), GetColorU32(ImGuiCol_Border), style.FrameRounding, ImDrawCornerFlags_All, 1.0f);
	...
	if (preview_value != NULL)
		RenderTextClipped(frame_bb.Min + style.FramePadding, value_bb.Max, preview_value, NULL, NULL, ImVec2(0.5f, 0.5f));
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
	...
	if (!BeginCombo(label, preview_text, ImGuiComboFlags_HeightLargest))
		return false;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	RenderTextClipped(bb_with_spacing.Min, bb_with_spacing.Max, label, NULL, &label_size, ImVec2(0.55f, 0.5f));
